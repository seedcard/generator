...
def generate_xpub(seed_words):
    ...
    seed_bytes = mnemonic_to_seed(seed_words)
...
def main():
    ...
    if choice == 'y':
        ...
        if choice == 'y':
            words = get_bip39_words(bin_chunks)
            ...
            if choice == 'y':
                xpub, bip44_acc = generate_xpub(words)
                print(f"\nxpub Key: {xpub}")
                # create_qr_code(xpub, filename="xpub_qr.png")

                fingerprint = compute_fingerprint(bip44_acc)
